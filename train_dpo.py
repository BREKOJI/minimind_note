...
def logits_to_probs(logits, labels):
    ...
    # 这里用的log_softmax就是softmax加log
    log_probs = F.log_softmax(logits, dim=2)
    # label (bsz, seqlen) -> (bsz, seqlen, 1)
    # torch.gather: 以index为索引提取log_probs在dim这一维度上的值
    # squeeze(-1)表示移除最后一个维度
...
def dpo_loss(ref_probs, probs, beta):
    ...
    # 在train_epoch刚开始，将chosen和rejected的batch cat到了一起，
    # dataloader返回的每个item都是
    #        return {
    #           'x_chosen': x_chosen,
    #           'y_chosen': y_chosen,
    ...
    #        }
    # 可以保证每个item都有一个chosen和rejected，所以一个batch的chosen和rejected是等量的
    # 所以cat后前面一半是chosen后面一半是rejected
    ...
    # beta越大对参考模型和当前模型偏好差异的程度越敏感
    # 这个loss function的意义是让当前模型更倾向于chosen回答对齐人类偏好，而又不会偏离参考模型太远，保证了不会过度优化或者模型退化
...
def train_epoch(epoch, wandb):
    ...
    for step, batch in enumerate(train_loader):
        ...
        # 把chosen的batch和rejected的batch cat到一起
        ...
        with ctx:
            ...
            # ref--(batch_size, seq_len, vocab_size) 
            # 每个元素代表一个概率，logits_to_probs表示得到logits中与y对应的结果的概率
            ...
            probs = probs * mask
            # 参考模型的预测偏好和当前模型的预测偏好的差异乘beta再取平均值
